...
int main(int argc, char * argv[])
{
    ...
    auto spinner = std::thread([&executer]() {executer.spin();});

    auto arm = moveit::planning_interface::MoveGroupInterface(node,"arm_group");
    arm.setMaxVelocityScalingFactor(1.0);
    arm.setMaxAccelerationScalingFactor(1.0);

    auto gripper = moveit::planning_interface::MoveGroupInterface(node,"gripper_group");
